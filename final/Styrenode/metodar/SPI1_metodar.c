...
// gyroskopet L3GD20 på plattformkortet.
//---------------------------------------
...
//------------------------------------------
  ...
  //Slepp først til klokka paa SPI1.
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_SPI1, ENABLE); // SPI1 og 2 er på to ulike bussar.
  ...
  //Oppsett av SPI2
	...
	SPI_InitStructure.SPI_CPHA = SPI_CPHA_2Edge;                /*!< Specifies the clock active edge for the bit capture.
                                         Pos flanke blir 2. flanke når clk er aktiv låg */
...
//Så oppsett av GPIO-pinnane PA5 (SCLK), 6 (MISO) og 7 (MOSI) som blir brukt av SPI1-modulen
//------------------------------------------
//Deklarasjon av initialiseringsstrukturen.
...
//Så oppsett av GPIO-pinnen PE3 (CS) som blir brukt av SPI1-modulen
//------------------------------------------
    //Deklarasjon av initialiseringsstrukturen.
...
//        SPI2->DR = data;
	...
	SPI_SendData8(SPI1, 0x0F); //PD=1 og X,Y,Z på (1)
	while(SPI1->SR & SPI_SR_FTLVL);
	while(SPI1->SR & SPI_SR_BSY);
    GPIOE->ODR = GPIOE->ODR | GPIO_Pin_3;
...
//	SPI_SendData8(SPI1, 0xAA); //Vilkårleg
//	while(SPI1->SR & SPI_SR_FTLVL);
//	while(SPI1->SR & SPI_SR_BSY);
//    GPIOE->ODR = GPIOE->ODR | GPIO_Pin_3;
